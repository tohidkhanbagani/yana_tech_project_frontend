function getManagerTimesheetsTemplate() {
        ...
        sortedTasks
          .map((t) => {
            ...
            const editedBadge = t.is_edited ? `
                <span class="inline-flex items-center gap-0.5 text-[8px] font-bold bg-amber-50 text-amber-755 border border-amber-200 px-1.5 py-0.5 rounded shrink-0">
            ...
            return `
                    <tr class="hover:bg-slate-50 border-b border-slate-100 last:border-0 cursor-pointer group" onclick="openTaskDetailsModal('${t.id}')">
                        <td class="py-2.5 px-4 whitespace-nowrap overflow-hidden">
                            <div class="text-xs font-bold text-slate-800 group-hover:text-brand-primary transition-colors truncate" title="${empName}">${empName}</div>
                        ...
                        <td class="py-2.5 px-4 whitespace-nowrap overflow-hidden text-xs font-bold text-slate-800">${parseFloat(t.hours_logged).toFixed(1)} <span class="text-[10px] text-slate-400 font-normal">hrs</span></td>
                    </tr>
                `;
          })
          .join("") ||
        `<tr><td colspan="4" class="px-4 py-8 text-center text-slate-500">No timesheets match selected filters.</td></tr>`;

      return `
                <div class="flex justify-between items-center mb-6">
                    <div>
                        <h3 class="text-2xl font-bold text-slate-800">Timesheets & Task Logs</h3>
                        <p class="text-slate-500 mt-1">Universal view of all logged hours and task descriptions.</p>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl border border-slate-200 p-3.5 mb-6 flex flex-col lg:flex-row lg:items-center justify-between gap-4 shadow-sm">
                    <div class="flex items-center gap-2 text-slate-700 shrink-0">
                        <div class="p-1.5 bg-indigo-50 text-indigo-655 rounded-lg">
                    ...
                    <div class="flex flex-wrap items-center gap-3">
                        ...
                        <div class="flex items-center gap-2 bg-slate-50 border border-slate-200/80 px-2.5 py-1.5 rounded-lg shadow-inner">
                            ...
                            <select id="timesheetsMonth" class="bg-transparent border-none text-xs font-bold text-slate-700 outline-none cursor-pointer focus:ring-0">
                            ...
                            <select id="timesheetsYear" class="bg-transparent border-none text-xs font-bold text-slate-700 outline-none cursor-pointer focus:ring-0">
                            ...
                            <button onclick="fetchCustomTimesheetsByMonthYear()" class="px-2.5 py-1 bg-slate-800 hover:bg-slate-750 text-white text-[10px] font-bold rounded transition-colors shadow-sm">Apply</button>
                        </div>
                        ...
                        <div class="flex items-center gap-2 bg-slate-50 border border-slate-200/80 px-2.5 py-1.5 rounded-lg shadow-inner">
                            ...
                            <input type="date" id="timesheetsStartDate" value="${state.timesheetsStartDate || ''}" class="bg-transparent border-none text-xs font-bold text-slate-755 outline-none cursor-pointer w-[110px]" title="Start Date">
                            <span class="text-xs text-slate-400 font-bold">&rarr;</span>
                            <input type="date" id="timesheetsEndDate" value="${state.timesheetsEndDate || ''}" class="bg-transparent border-none text-xs font-bold text-slate-755 outline-none cursor-pointer w-[110px]" title="End Date">
                            <button onclick="fetchCustomTimesheetsData()" class="px-2.5 py-1 bg-brand-primary hover:bg-indigo-700 text-white text-[10px] font-bold rounded transition-colors shadow-sm">Fetch</button>
                            <button onclick="clearTimesheetFilters()" class="px-2.5 py-1 bg-slate-200 hover:bg-slate-300 text-slate-700 text-[10px] font-bold rounded transition-colors shadow-sm flex items-center gap-1" title="Clear Filters">
                        ...
                        <table class="w-full text-left table-fixed border-collapse">
                            <thead class="bg-slate-50/50 border-b border-slate-200">
                                <tr class="text-slate-500 text-[10px] font-black uppercase tracking-wider">
                                    <th class="py-2.5 px-4 w-[30%]">Employee & Date</th>
                                    <th class="py-2.5 px-4 w-[30%]">Project</th>
                                    <th class="py-2.5 px-4 w-[25%]">Task Type</th>
                                    <th class="py-2.5 px-4 w-[15%]">Hours</th>
                                </tr>
                            </thead>
